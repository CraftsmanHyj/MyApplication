 ...
 *
 * @Author hyj
 * @Date 2015-12-21 下午6:02:40
 */
public class CharacterParser {
    /**
     * 常用汉字对应的ASCII值
     */
    private static final int[] pyvalue = new int[]{-20319, -20317, -20304,
     ...
     * 常用汉字对应的拼音
     */
    public static final String[] pystr = new String[]{"a", "ai", "an", "ang",
    ...
    /**
     * 单字解析成ASCII码，返回对应汉字的拼音
     *
     * @param ch 要转换的字符串
     * @return
     */
    private String convert(String ch) {
        String result = null;
        int ascii = getChsAscii(ch);
    ...
    /**
     * 只获取每个汉字/字母开头的第一个字母<br/>
     * 如果为在常量数组里面找到对应的拼音值，则拼音返回的是：unknown
     *
     * @param str       要转换的字符串
     * @param onlyStart ture：只取每个汉字拼音开头一个字母；false：取每个汉字全拼；
     * @return
     */
    public String getSelling(String str, boolean onlyStart) {
        String key, value;
        ...
        for (int i = 0; i < str.length(); i++) {
            ...
            value = onlyStart ? value.substring(0, 1) : value;
            buffer.append(value);
        }
