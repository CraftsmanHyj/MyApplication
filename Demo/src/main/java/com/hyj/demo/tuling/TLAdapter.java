...
public class TLAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (0 == layoutID) {
            holder = ViewHolder.getInstance(context, convertView, parent, R.layout.tuling_item_incoming, position);
            holder.setText(R.id.tlInDate, chat.getDateStr());
            holder.setText(R.id.tlInTvMsg, chat.getMsg());
        } else {
            holder = ViewHolder.getInstance(context, convertView, parent, R.layout.tuling_item_outcoming, position);
