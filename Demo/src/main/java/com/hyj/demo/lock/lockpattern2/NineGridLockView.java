package com.hyj.demo.lock.lockpattern2;
...
/**
 * <pre>
 * 九宫格锁 关于画外圆，可以先直接算出圆心点和半径，以绘制。
 * 本例先算的是圆的外矩形 连线时如果感觉突兀，可记录内圆心点，以内圆心作碰撞检测
 * 可以完善的地方：保存点的顺序，onmeasure中，判断是否匹配，用于解锁方案
 * </pre>
 */
public class NineGridLockView extends View {
    private final int mCount = 3; // n行n列
    // 内圆半径;
    private float mMinRadius = TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_DIP, 10, getResources().getDisplayMetrics());
    // 绘制时的画笔宽度
    private float mStrokeWidth = TypedValue.applyDimension(
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < mCount; i++) {
            for (int j = 0; j < mCount; j++) {
                rectF = new RectF(mRadius + 3 * j * mRadius, mRadius + 3 * i
                        * mRadius, mRadius + 3 * j * mRadius + rectWH, mRadius
                        + 3 * i * mRadius + rectWH);
                mListRectFs.add(rectF);
                // item外框
                // canvas.drawRect(rectF, mPaint);
                // canvas.drawArc(rectF, 0, 360, true, mPaint);
                mListCircle.add(new Point((int) (rectF.left + mRadius),
                        (int) (rectF.top + mRadius)));
            }
        }
    }
     ...
     * @return
     */
    private boolean isCollision(float x1, float y1, float x2, float y2,
                                float radius) {
