...
import com.hyj.demo.http.download.ThreadInfo;
...
public class ThreadDaoImpl implements ThreadDao {
    ...
    @Override
    public Map<String, Integer> queryFileProgress() {
        Map<String, Integer> mapProgress = new HashMap<String, Integer>();
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        String sql = "select  url,(sum(progress)*100/max(end)) progress from thread_info group by url";
        Cursor cursor = db.rawQuery(sql, null);
        while (cursor.moveToNext()) {
            String url = cursor.getString(cursor.getColumnIndex("url"));
            int progress = cursor.getInt(cursor.getColumnIndex("progress"));
            mapProgress.put(url, progress);
        }
        return mapProgress;
    }
